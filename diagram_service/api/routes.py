...
from diagram_service.agents.diagram_agent import DiagramAgent, DiagramAgentPool
from diagram_service.api.models import (
...
agent_pool: Optional[DiagramAgentPool] = None
...
def set_agent_pool(pool: DiagramAgentPool) -> None:
...
async def generate_diagram(
    ...
    try:
        ...
        async def create_diagram(agent: DiagramAgent) -> str:
        ...
        async def get_spec(agent: DiagramAgent):
...
async def assistant_chat(
    request: AssistantChatRequest, pool: DiagramAgentPool = Depends(get_agent_pool)
) -> AssistantChatResponse:
    """
    ...
    try:
        ...
        async def chat_with_agent(agent: DiagramAgent):
...
async def health_check(
    ...
    try:
        ...
        async def check_health(agent: DiagramAgent):
        ...
        return HealthResponse(
            status="unhealthy", components={"error": False}, supported_node_types=[]
        )
...
async def get_supported_components(
    ...
    try:
        ...
        async def get_components(agent: DiagramAgent):
            return agent.get_supported_components()

        # Execute with agent from pool
        components = await pool.execute_with_agent(get_components)

        return {"supported_components": components, "total_count": len(components)}

    except Exception as e:
        logging.error(f"Error getting supported components: {e}")
        raise HTTPException(
            status_code=500, detail=f"Failed to get supported components: {str(e)}"
        )
